    ...
    <div class="balance_part_header">
      ...
      <div class="balance_info">
        <div class="account">
          ...
          <div class="info">
            <div>{{$t("balance_list.balance")}}</div>
            <div>{{deliveryList.accountBalance || 0}}P</div>
          </div>
        </div>
        <div class="account">
          <div class="icon_colors"><i class="iconfont icon-xinyong"></i></div>
          <div class="info">
            <div>{{$t("balance_list.xyBalance")}}</div>
            <div>{{deliveryList.creditAccountBalance || 0}}P</div>
          </div>
        </div>
      </div>
    </div>
    <div class="balance_part">
      ...
      <div class="balance_account">
        <div>
          ...
          <span>{{income.srMoney || 0}}P</span>
        </div>
        <div>
          <span v-if="isFull==1">{{$t("balance_list.balance1")}}:</span>
          <span v-else>{{$t("balance_list.balance2")}}:</span>
          <span>{{income.zhMoney || 0}}P</span>
        </div>
        <div>
          <span v-if="isFull==1">{{$t("balance_list.balance3")}}:</span>
          <span v-else>{{$t("balance_list.balance4")}}:</span>
          <span>{{income.xyMoney || 0}}P</span>
        </div>
      </div>
      <el-table :data="balanceData" stripe style="width: 100%" v-loading="loading" v-if="isFull==1" :key="isFull" @sort-change="tableSortChange">
        <el-table-column prop="AccountType" :label="$t('balance_list.info')"></el-table-column>
        <el-table-column prop="money" :label="$t('balance_list.income_balance')" sortable='custom'></el-table-column>
        <el-table-column prop="createTime" :label="$t('balance_list.time')" min-width="130" sortable='custom'></el-table-column>
        ...
        <el-table-column prop="accountBalanceType" :label="$t('balance_list.type')"></el-table-column>
      </el-table>
      ...
      store
        ...
        .then(res => {
          ...
          if (res.data.code == 1) {
            ...
            let srMoney = res.data.data.srMoney;
            let xyMoney = res.data.data.xyMoney;
            let zhMoney = res.data.data.zhMoney;
            this.income = {
              srMoney, xyMoney, zhMoney
            };
      ...
      store
        ...
        .then(res => {
          ...
          if (res.data.code == 1) {
            ...
            let srMoney = Math.abs(res.data.data.zcMoney);
            let xyMoney = Math.abs(res.data.data.xyMoney);
            let zhMoney = Math.abs(res.data.data.zhMoney);
            this.income = {
              srMoney, xyMoney, zhMoney
            };
            console.log(res.data.data.zcMoney, "income")
            this.balanceAllData = res.data.data.expenditureList;
            // this.deliveryList = res.data.data.deliveryList;
            this.total = res.data.data.totalResult;
            console.log(res.data.data.totalResult, 11111111111111)
          } else {
            this.loading = false;
            this.$message.error(res.data.message);
          }
